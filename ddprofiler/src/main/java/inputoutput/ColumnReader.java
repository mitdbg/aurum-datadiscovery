...
public class ColumnReader implements Iterable<Record> {
  ...
  private static final Logger log =
      Logger.getLogger(ColumnReader.class.getName());

  private Connector conn;
  private Config config;

  public ColumnReader() { config = new Config(); }
  ...
  public void initConnector(Config config) {
    ...
    if (config.getConn_type().equals("FILE")) {
      ...
      try {
        conn =
            new FileConnector(config.getConn_path(), config.getConn_filename(),
                              config.getSpliter());
      } catch (IOException e) {
      ...
      try {
        ...
        conn = new DBConnector(null, config.getConn_ip(), config.getPort(),
                               config.getConn_path(), config.getConn_filename(),
