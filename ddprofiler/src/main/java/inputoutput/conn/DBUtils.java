...
public class DBUtils {
  ...
  public static Properties loadDBPropertiesFromFile() {
    ...
    try {
      InputStream is = DBUtils.class.getClassLoader()
                           .getResource("dbconnector.config")
                           .openStream();
      prop.load(is);
    } catch (IOException e) {
      e.printStackTrace();
  ...
  private static Connection getMYSQLConnection(String connIP, String port,
                                               String dbName, String username,
                                               String password) {
    Connection conn = null;
    try {
      Class.forName("com.mysql.jdbc.Driver");
      conn = DriverManager.getConnection("jdbc:mysql://" + connIP + ":" + port +
                                             "/" + dbName,
                                         username, password);
  ...
  private static Connection getPOSTGRESQLConnection(String connIP, String port,
                                                    String dbName,
                                                    String username,
                                                    String password) {
    Connection conn = null;
  ...
  private static Connection getOracle10GConnection(String connIP, String port,
                                                   String dbName,
                                                   String username,
                                                   String password) {
    Connection conn = null;
