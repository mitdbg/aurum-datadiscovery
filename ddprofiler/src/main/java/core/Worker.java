...
public class Worker implements Runnable {
  ...
  public Worker(Conductor conductor, ProfilerConfig pc, String workerName,
                BlockingQueue<TaskPackage> taskQueue,
                BlockingQueue<ErrorPackage> errorQueue, Store store,
                EntityAnalyzer cached) {
  ...
  public void stop() { this.doWork = false; }
  ...
  private WorkerTask pullTask() {
    ...
    try {
      ...
      if (tp.getType() == TaskPackageType.CSV) {
        wt = WorkerTask.makeWorkerTaskForCSVFile(tp.getPath(), tp.getName(),
                                                 tp.getSeparator());
      } else if (tp.getType() == TaskPackageType.DB) {
        wt = WorkerTask.makeWorkerTaskForDB(
            tp.getDBType(), tp.getIp(), tp.getPort(), tp.getDBName(),
            tp.getStr(), tp.getUsername(), tp.getPassword());
      }
  ...
  public void run() {
    ...
    while (doWork) {
      try {
        ...
        DataIndexer indexer = new FilterAndBatchDataIndexer(
            store, task.getConnector().getSourceName());
        ...
        WorkerTaskResultHolder wtrf = new WorkerTaskResultHolder(
            c.getSourceName(), c.getAttributes(), analyzers);

        //				List<WorkerTaskResult> rs =
        //WorkerTaskResultHolder.makeFakeOne();
        //				WorkerTaskResultHolder wtrf = new
        //WorkerTaskResultHolder(rs);
  ...
  private void readFirstRecords(Map<Attribute, Values> initData,
                                Map<String, Analysis> analyzers,
                                DataIndexer indexer) {
    for (Entry<Attribute, Values> entry : initData.entrySet()) {
      ...
      if (at.equals(AttributeType.FLOAT)) {
        ...
        ((NumericalAnalysis)analysis)
            .feedFloatData(entry.getValue().getFloats());
      } else if (at.equals(AttributeType.INT)) {
        analysis = AnalyzerFactory.makeNumericalAnalyzer();
        ((NumericalAnalysis)analysis)
            .feedIntegerData(entry.getValue().getIntegers());
  ...
  private void feedValuesToAnalyzers(Map<Attribute, Values> data,
                                     Map<String, Analysis> analyzers) {
    // Do the processing
    for (Entry<Attribute, Values> entry : data.entrySet()) {
      ...
      if (vs.areFloatValues()) {
        ((NumericalAnalysis)analyzers.get(atName))
            .feedFloatData(vs.getFloats());
      } else if (vs.areIntegerValues()) {
        ((NumericalAnalysis)analyzers.get(atName))
            .feedIntegerData(vs.getIntegers());
