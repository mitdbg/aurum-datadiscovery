...
public class WorkerTaskResultHolder {
  ...
  public static List<WorkerTaskResult> makeFakeOne() {
    ...
    WorkerTaskResult wtr = new WorkerTaskResult(-1, "none", "none", "N", 100,
                                                100, 0, 100, 50, 50, 50);
    rs.add(wtr);
    return rs;
  }
  ...
  public WorkerTaskResultHolder(String sourceName, List<Attribute> attributes,
    ...
    for (Attribute a : attributes) {
      ...
      if (at.equals(AttributeType.FLOAT)) {
        ...
        WorkerTaskResult wtr = new WorkerTaskResult(
            id, sourceName, a.getColumnName(), "N",
        ...
        WorkerTaskResult wtr = new WorkerTaskResult(
            id, sourceName, a.getColumnName(), "T",
            (int)ta.getCardinality().getTotalRecords(),
            (int)ta.getCardinality().getUniqueElements(), entities);
        rs.add(wtr);
      }
    }
    this.results = rs;
  ...
  public List<WorkerTaskResult> get() { return results; }
}
