...
public class FilterAndBatchDataIndexer implements DataIndexer {
  ...
  private void storeNewValuesAndMaybeTriggerIndex(int id, String dbName, String path, String sourceName,
                                                  Attribute a,
  ...
  public boolean flushAndClose() {
    for (Entry<Attribute, Long> entry : attributeIds.entrySet()) {
      Attribute a = entry.getKey();
      store.indexData(entry.getValue(), dbName, path, sourceName, a.getColumnName(),
