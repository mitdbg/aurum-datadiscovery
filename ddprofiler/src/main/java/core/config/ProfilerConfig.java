...
public class ProfilerConfig extends Config {
    ...
    private static final String EXECUTION_MODE_DOC = "(online - 0) for server mode and (offline files - 1) for one-shot read files from directory and (offline db - 2) for one-shot read tables from db, (benchmark 3) for benchmarking purposes";
    ...
    static {
	config = new ConfigDef().define(DB_NAME, Type.STRING, "", Importance.HIGH, DB_NAME_DOC)
