...
public class ConfigDef {
  ...
  private final Map<String, ConfigKey> configKeys =
      new HashMap<String, ConfigKey>();

  public ConfigKey getConfigKey(String name) { return configKeys.get(name); }
   ...
   * @param validator A validator to use in checking the correctness of the
   * config
   * @param importance The importance of this config: is this something you will
  ...
  public ConfigDef define(String name, Type type, Object defaultValue,
    ...
    configKeys.put(name, new ConfigKey(name, type, parsedDefault, validator,
  ...
  public ConfigDef define(String name, Type type, Validator validator,
                          Importance importance, String documentation) {
    return define(name, type, NO_DEFAULT_VALUE, validator, importance,
                  documentation);
  }
   ...
   * is a map of configs. It is expected
   * that the keys of the map are strings, but the values can either be strings
   * or they may already be of the
   * appropriate type (int, string, etc). This will work equally well with
   * either java.util.Properties instances or a
   * programmatically constructed map.
   * @param props The configs to parse and validate
   * @return Parsed and validated configs. The key will be the config name and
   * the value will be the value parsed into
    ...
    for (ConfigKey key : configKeys.values()) {
      ...
      if (props.containsKey(key.name))
        ...
        throw new ConfigException("Missing required configuration \"" +
                                  key.name + "\" which has no default value.");
      else
    ...
    try {
      ...
      case STRING:
        if (value instanceof String)
          ...
          throw new ConfigException(
              name, value, "Expected value to be a string, but it was a " +
      ...
      case INT:
        if (value instanceof Integer) {
          ...
          throw new ConfigException(name, value,
                                    "Expected value to be an number.");
        }
      case LONG:
        if (value instanceof Integer)
  ...
  public enum Type {
    ...
    Type(Class<?> clazz) { this.clazz = clazz; }

    public Class<?> ofClass() { return this.clazz; }
  }
   ...
   */
  public interface Validator { public void ensureValid(String name, Object o); }
  ...
  public static class Range implements Validator {
     ...
     */
    public static Range atLeast(Number min) { return new Range(min, null); }
  ...
  public static class ValidString implements Validator {
    ...
    public String toString() { return "[" + join(validStrings) + "]"; }
  ...
  public String toHtmlTable() {
    ...
    List<ConfigKey> configs =
        new ArrayList<ConfigKey>(this.configKeys.values());
